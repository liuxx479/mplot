...
#inline
#python s bins ngal interpmethod mask (a percentage 0<mask<1)
########## constants & input ######
...
#analytical_dir = '/direct/astro+astronfs01/workarea/jia/fit_noise_sim45_mask/'
#analytical_dir = '/direct/astro+astronfs01/workarea/jia/fit_noise_sim45_mask_NoScaleX/'
analytical_dir = '/direct/astro+astronfs01/workarea/jia/fit_noise_sim45_mask2_NoScaleX/'
peak_dir = '/direct/astro+astronfs01/workarea/jia/peaks_noise_sim45/'
#peaks_mat_dir = '/direct/astro+astronfs01/workarea/jia/peaks_mat_noise_sim45_mask/'
peaks_mat_dir = '/direct/astro+astronfs01/workarea/jia/peaks_mat_noise_sim45_mask2/'
...
#peaks_mat_fidu_dir = '/direct/astro+u/jia/magbias/peaks_mat_noise_mask/'#5sim
peaks_mat_fidu_dir = '/direct/astro+astronfs01/workarea/jia/peaks_mat_noise_mask2/'#5sim
...
def batch_kmap_noise(kmap_dir = kmap_sim45_dir, peaks_mat_dir = peaks_mat_dir, cosmo_num = contr, s=s, bins=bins, sigma=sigma, bias=0, mask=0):#add mask=0
	...
	if mask == 0:
		...
		if bias:
			file_matrix = peaks_mat_dir+'peak_matrix_bias_'+str(kmapnum)+cosmo+'_s%s_g%s_ngal%s_bins%s_mask%s.ls'%(s,sigma,int(ngal_arcmin),bins,mask)
		else:
	...
	if os.path.isfile(file_matrix):
		...
		for n in range(len(kmaps)):
			...
			if mask != 0:# add masking
				numpeaks=int(len(peaks)*(1.0-mask))
				randidx=np.random.permutation(peaks)
				
				#peaks=np.random.choice(peaks,numpeaks)
				peaks = randidx[:numpeaks]
...
true_mat_noise = batch_kmap_noise(kmap_dir = kmap_sim45_dir,cosmo_num = contr, bias=0, mask=mask)
bias_mat_noise = batch_kmap_noise(kmap_dir = kmap_sim45_dir,cosmo_num = contr, bias=1, mask=mask)
...
for i in np.arange(len(cosmo_all)):
	...
	true_mat_noise_all[i] = batch_kmap_noise(kmap_dir = kmap_dir, peaks_mat_dir = peaks_mat_fidu_dir, cosmo_num = i, bias=0, mask=mask)
...
dNdw =dNw/dp[1] 
dNdsi=dNsi/dp[2]
X=np.mat([dNdOm,dNdw,dNdsi])[:,:num]#*(1.0-mask)


def analytical_fits(peak_mat, s=s, bins=bins, sigma=sigma, bias=1):
	if bias:
		fit_filename = analytical_dir+'bias_analytical_s%s_g%s_ngal%s_bins%s_mask%s.ls'%(s,sigma,int(ngal_arcmin),bins,mask)
	else:
